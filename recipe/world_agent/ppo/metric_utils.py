...
def flatten_nested_metrics(metrics: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert nested metrics with multiple slashes to wandb-friendly format.
    
    Examples:
        "episode/reward/mean" -> "episode/reward_mean"
        "episode/length/max" -> "episode/length_max"
        "episode/tool_call_count/min" -> "episode/tool_call_count_min"
    
    Args:
        metrics: Dictionary with potentially nested metric names
        
    Returns:
        Dictionary with flattened metric names
    """
    flattened = {}
    ...
    for key, value in metrics.items():
        ...
        if slash_count >= 2:
            ...
            # Keep the first part, join the rest with underscore
            if len(parts) >= 3:
            ...
            # Keep single-level metrics as is
            flattened[key] = value
