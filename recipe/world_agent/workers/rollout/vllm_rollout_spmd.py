...
def traj_fingerprint(input_ids, k=32):
    arr = np.asarray(input_ids[:k], dtype=np.int32)
    return hashlib.sha256(arr.tobytes()).hexdigest()


class TokenStatRecorder:
    def __init__(self):
        self.data = defaultdict(list)

    def record(self, request_id, step, entropy, max_logprob, score):
        self.data[request_id].append({
            "step": step,
            "entropy": entropy,
...
class ThinkLogitsProcessor:
    """
    Limit the length of <think> section; and trigger stop when max log prob is almost unchanged for several steps.
    """
    ...
    def __init__(
        ...
        combine_alpha: float = 0.6,
        record_token_min_length: int = 32,
        recorder: TokenStatRecorder = None,
    ):
        ...
        self._last_max_logprob = []
        self._stagnation_count = []
        self.recorder = recorder
        self.record_token_min_length = record_token_min_length
    ...
    def __call__(self, input_ids, logits: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(batch_size):
            ...
            if self.enable_confidence_stop:
                logprob = torch.log_softmax(logits[b], dim=-1)
                ...
                score = alpha * max_logprob + (1.0 - alpha) * one_minus_entropy

                # Since vLLM cannot guarantee the order of the tokens, 
                # We need to use the fingerprint to record the trajectory.
                # Fingerprint is calculated by the hash code of the first 32 tokens.
                # Thus, in parctice, we only use the token entropy after the first 32 tokens to calculate the turn-level signal.
                if len(ids_b) > self.record_token_min_length:
                    traj_id = traj_fingerprint(ids_b)
                ...
                self._last_max_logprob[b] = score
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer, model_hf_config, **kwargs):
        ...
        super().__init__()
        self.config = config
        self.recorder = TokenStatRecorder()
        ...
        self.sampling_params = SamplingParams(
            logits_processors=[
                ThinkLogitsProcessor(
                    num_think_tokens=config.num_think_tokens,
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            turn_level_score = np.zeros(batch_size, dtype=np.float32)
            for batch_idx, seq_key in enumerate(self.recorder.data.keys()):
                if batch_idx >= batch_size:
                    break
                if self.recorder.data[seq_key] != []:
                    traj_score = 0
                    if len(self.recorder.data[seq_key]) > 0:
                        for i in range(len(self.recorder.data[seq_key])):
                            traj_score += self.recorder.data[seq_key][i]['score']
                        traj_score /= len(self.recorder.data[seq_key])
                    turn_level_score[batch_idx] = traj_score
        ...
        # Considering some responses are same with first 32 tokens,
        # we need to match the fingerprint of our recorded trajectory with the response.
        # For those same responses, we just set turn_level_score as zero.
        turn_level_score_sorted = np.zeros(batch_size, dtype=np.float32)
        for i in range(batch_size):
            response_ids = response.detach().cpu().numpy()[i]
            fingerprint = traj_fingerprint(response_ids)
            for idx_key, key in enumerate(self.recorder.data.keys()):
                if idx_key >= batch_size:
                    break
                if key == fingerprint:
                    turn_level_score_sorted[i] = turn_level_score[idx_key]
                    # print(f"fingerprint matches self.recorder.data.keys()[{idx_key}]")
                    break
        non_tensor_batch['turn_level_score'] = turn_level_score_sorted
